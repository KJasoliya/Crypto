...
if (isset($_POST['submitDeleteRecord'])) {
    include 'config.php';
    if (!empty($_POST['selectedCipher'])) {
        echo "In func";
        foreach ($_POST['selectedCipher'] as $selected) {
            echo $selected;
            $query = mysqli_query($con, "DELETE FROM `userhistory` WHERE `id` = \"" . $selected . "\";");
        }
...
if (isset($_POST['yesDeactivateAccount'])) {
    include 'config.php';
    session_start();
    $query = mysqli_query($con, "DELETE FROM `userinfo` WHERE `email` = \"" . $_SESSION['userEmail'] . "\";");
    $query = mysqli_query($con, "DELETE FROM `userhistory` WHERE `email` = \"" . $_SESSION['userEmail'] . "\";");
    session_destroy();
    header("Location: ciphers.php");
}
if (isset($_POST['cancelDeactivateAccount'])) {
    header("Location:dashboard.php");
}
